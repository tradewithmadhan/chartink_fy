...
function getCustomSeriesPlotRow(time, index, item, originalTime, dataToPlotRow) {
    ...
    // Preserve OHLC when custom series provides [open, high, low, close]
    // Otherwise, fallback to single-value semantics [last, max, min, last]
    let value;
    if (Array.isArray(values) && values.length === 4 && values.every((v) => typeof v === 'number' && Number.isFinite(v))) {
        // Treat as OHLC in order: open, high, low, close
        value = [values[0], values[1], values[2], values[3]];
    } else {
        const max = Math.max(...values);
        const min = Math.min(...values);
        const last = values[values.length - 1];
        value = [last, max, min, last];
    }
